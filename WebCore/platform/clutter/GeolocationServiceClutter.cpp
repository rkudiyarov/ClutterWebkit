...
#include "config.h"
#include "GeolocationServiceClutter.h"
...
GeolocationService* GeolocationServiceClutter::create(GeolocationServiceClient* client)
{
    return new GeolocationServiceClutter(client);
}

GeolocationService::FactoryFunction* GeolocationService::s_factoryFunction = &GeolocationServiceClutter::create;

GeolocationServiceClutter::GeolocationServiceClutter(GeolocationServiceClient* client)
...
GeolocationServiceClutter::~GeolocationServiceClutter()
{
    if (m_geoclueClient)
...
//       API for that.
//
bool GeolocationServiceClutter::startUpdating(PositionOptions* options)
{
...
void GeolocationServiceClutter::stopUpdating()
{
    if (!m_geoclueClient)
        return;
...
void GeolocationServiceClutter::suspend()
{
    // not available with geoclue
    notImplemented();
}

void GeolocationServiceClutter::resume()
{
    // not available with geoclue
    notImplemented();
}

Geoposition* GeolocationServiceClutter::lastPosition() const
{
    return m_lastPosition.get();
}

PositionError* GeolocationServiceClutter::lastError() const
{
    return m_lastError.get();
}

void GeolocationServiceClutter::updatePosition()
{
    m_lastError = 0;
...
void GeolocationServiceClutter::getPositionCallback(GeocluePosition *position,
                                                ...
                                                GError* error,
                                                GeolocationServiceClutter* that)
{
    if (error) {
...
void GeolocationServiceClutter::position_changed(GeocluePosition*, GeocluePositionFields fields, int timestamp, double latitude, double longitude, double altitude, GeoclueAccuracy* accuracy, GeolocationServiceClutter* that)
{
...
void GeolocationServiceClutter::setError(PositionError::ErrorCode errorCode, const char* message)
{
    m_lastPosition = 0;

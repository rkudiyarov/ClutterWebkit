...
#include "FrameNetworkingContextClutter.h"
...
PasteboardHelperClutter* pasteboardHelperInstance()
{
    static PasteboardHelperClutter* helper = new PasteboardHelperClutter();
    return helper;
}
...
static ClutterActor* currentToplevelCallback(WebKitSoupAuthDialog* feature, SoupMessage* message, gpointer userData)
{
    ...
    WebKit::FrameNetworkingContextClutter* context = static_cast<WebKit::FrameNetworkingContextClutter*>(d->m_context.get());
    if (!context)
        return NULL;
    ...
    ClutterActor* toplevel =  clutter_actor_get_stage(CLUTTER_ACTOR(context->coreFrame()->page()->chrome()->platformPageClient()));
    return toplevel;
}
...
GSettings* inspectorGSettings()
{
    ...
    const gchar* schemaID = "org.webkit-clutter-"WEBKITCLUTTER_API_VERSION_STRING".inspector";

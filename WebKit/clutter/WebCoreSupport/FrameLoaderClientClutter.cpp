...
static WebKitWebNavigationAction* getNavigationAction(const NavigationAction& action, const char* targetFrame)
{
    ...
    if (keyStateEvent) {
        if (keyStateEvent->shiftKey())
            modifierFlags |= CLUTTER_SHIFT_MASK;
        if (keyStateEvent->ctrlKey())
            modifierFlags |= CLUTTER_CONTROL_MASK;
        if (keyStateEvent->altKey())
            modifierFlags |= CLUTTER_MOD1_MASK;
        if (keyStateEvent->metaKey())
            modifierFlags |= CLUTTER_MOD2_MASK;
    }
...
void FrameLoaderClient::transitionToCommittedForNewPage()
{
    ...
    clutter_actor_get_size(CLUTTER_ACTOR(containingWindow), &awidth, &aheight);
    
    IntSize size = IntSize((int)awidth, (int)aheight);
...
{
    return FrameNetworkingContextClutter::create(core(m_frame));
}
